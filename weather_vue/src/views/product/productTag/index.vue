        ...
        <el-table :data="pageList" style="width: 100%; min-height: 400px">
          ...
          <el-table-column label="标签">
            <template #default="scope">
              <div v-if="scope.row.type == '自定义'">
                <div
                  class="custom-label"
                  :style="{
                    color: scope.row.fontColor,
                    backgroundColor: scope.row.bgColor,
                    borderColor: scope.row.borderColor,
                    border: '1px solid',
                    borderRadius: '4px',
                    padding: '4px 8px', // 内边距，可根据字体大小调整
                    boxSizing: 'border-box', // 确保边框和内边距包含在总宽度内
                    lineHeight: '1.5', // 根据需要调整行高
          ...
          <!--          <template #default="scope">-->
          <!--            <img :src="scope.row.image" alt="图片" style="max-width: 100px; max-height: 100px"/>-->
          <!--          </template>-->
          <!--        </el-table-column>-->
          <el-table-column prop="status" label="是否开启" width="130">
            <template #default="scope">
              <el-switch
          ...
          <el-table-column prop="mobileShow" label="移动端展示" width="150">
            <template #default="scope">
              <el-switch
                ...
                @change="
                  () => {
                    handleMobileShowChange(scope.row);
                  }
                "
              />
            </template>
...
import * as productTagInfoApi from "@/api/product/productTag/productTagInfoApi";
...
import SaveInfoForm from "@/views/product/productTag/SaveInfoForm.vue";
import SaveCategory from "@/views/product/productTag/SaveCategory.vue";
import AdvancedQuery from "@/views/product/productTag/AdvancedQuery.vue";
import { ElMessage, ElMessageBox, ElTree } from "element-plus";
import MePageAsidePanel from "@/components/PageLayout/MePageAsidePanel.vue";
import { getSetMobileShowApi } from "@/api/product/productTag/productTagInfoApi";
...
const handleStatusChange = async (row: any) => {
  ...
  // 发起编辑状态
  if (status == 0) {
    let res = await productTagInfoApi.getSetStatusApi({
      id: id,
      status: 0
    });
  }
...
//改变状态status
const handleMobileShowChange = async (row: any) => {
  const { id, mobileShow } = row;
  // 发起编辑状态
  if (mobileShow == 0) {
    let res = await productTagInfoApi.getSetMobileShowApi({
      id: id,
      mobileShow: 0
    });
  }
...
const onDelCategory = async (id: string) => {
  ...
  ElMessageBox.confirm("你确定删除吗？", "操作弹框", {
    ...
    .then(async () => {
      // 用户点击“确认”按钮时的回调
      let res = await productTagCategoryApi.getDelApi(id);
      console.log(res);
      if (res.success) {
        ElMessage({
          type: "success",
...
const getTree = async () => {
  ...
  const res = await productTagCategoryApi.getTreeApi(pageParams);
...
const getPage = async () => {
  ...
  const res = await productTagInfoApi.getPageApi(pageParams);
...
const handleDelete = async (id: string) => {
  ...
  ElMessageBox.confirm("你确定删除吗？", "操作弹框", {
    ...
    .then(async () => {
      // 用户点击“确认”按钮时的回调
      let res = await productTagInfoApi.getDelApi(id);
      console.log(res.success);
      if (res.success) {
        refreshData();
        ElMessage({
