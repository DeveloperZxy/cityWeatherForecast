    ...
    <MePageBasePanel>
      ...
      <template #main-content>
        <el-table :data="pageList" style="width: 100%; min-height: 400px">
          ...
          <el-table-column prop="names" label="规格名" />
          <el-table-column label="规格值">
            <template #default="scope">
              <div
                v-for="(attrStr, index) in scope.row.attributes"
                :key="index"
              >
                {{ attrStr }}
              </div>
...
import SaveInfoForm from "@/views/product/productSpec/SaveInfoForm.vue";
import SaveCategory from "@/views/product/productSpec/SaveCategory.vue";
import AdvancedQuery from "@/views/product/productSpec/AdvancedQuery.vue";
import { ElMessage, ElMessageBox, ElTree } from "element-plus";
import MePageAsidePanel from "@/components/PageLayout/MePageAsidePanel.vue";
import { getSetMobileShowApi } from "@/api/product/productSpec/productSpecInfoApi";
...
const onDelCategory = async (id: string) => {
  ...
  ElMessageBox.confirm("你确定删除吗？", "操作弹框", {
    ...
    .then(async () => {
      // 用户点击“确认”按钮时的回调
      let res = await productSpecCategoryApi.getDelApi(id);
      console.log(res);
      if (res.success) {
        ElMessage({
          type: "success",
...
const getPage = async () => {
  ...
  const res = await productSpecInfoApi.getPageApi(pageParams);
...
const handleDelete = async (id: string) => {
  ...
  ElMessageBox.confirm("你确定删除吗？", "操作弹框", {
    ...
    .then(async () => {
      // 用户点击“确认”按钮时的回调
      let res = await productSpecInfoApi.getDelApi(id);
      console.log(res.success);
      if (res.success) {
        refreshData();
        ElMessage({
