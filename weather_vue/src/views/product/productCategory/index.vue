    ...
    <MePageBasePanel>
      ...
      <template #main-content>
        ...
        >
          ...
          <el-table-column prop="image" label="分类图标" width="200">
...
import * as productCategoryInfoApi from "@/api/product/productCategory/productCategoryInfoApi";
import { Edit, Delete, Search, Plus } from "@element-plus/icons-vue";
import SaveInfoForm from "@/views/product/productCategory/SaveInfoForm.vue";
import AdvancedQuery from "@/views/product/productCategory/AdvancedQuery.vue";
...
const handleStatusChange = async (row: any) => {
  ...
  // 发起编辑状态
  if (status == 0) {
    let res = await productCategoryInfoApi.getSetStatusApi({
      id: id,
      status: 0
    });
  }
...
const getTree = async () => {
  ...
  const res = await productCategoryInfoApi.getTreeApi(pageParams);
...
const handleDelete = async (id: string) => {
  ...
  ElMessageBox.confirm("你确定删除吗？", "操作弹框", {
    ...
    .then(async () => {
      // 用户点击“确认”按钮时的回调
      let res = await productCategoryInfoApi.getDelApi(id);
