...
import {
  ...
  getEditApi,
  getShowApi
} from "@/api/cms/articleTag/articleTagInfoApi";
...
const getTree = async () => {
  const res = await articleTagCategoryApi.getTreeApi({});
...
const open = async (type: string, id?: any, categoryId?: any) => {
  ...
  let response = await articleTagCategoryApi.getListApi({ includeRoot: "0" });
