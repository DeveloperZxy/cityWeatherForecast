...
import * as articleTagInfoApi from "@/api/cms/articleTag/articleTagInfoApi";
import * as articleTagCategoryApi from "@/api/cms/articleTag/articleTagCategoryApi";

import { Edit, Delete, Search, Plus } from "@element-plus/icons-vue";
import SaveInfoForm from "@/views/cms/articleTag/SaveInfoForm.vue";
import SaveCategory from "@/views/cms/articleTag/SaveCategory.vue";
import AdvancedQuery from "@/views/cms/articleTag/AdvancedQuery.vue";
...
const handleStatusChange = async (row: any) => {
  ...
  // 发起编辑状态
  if (status == 0) {
    let res = await articleTagInfoApi.getSetStatusApi({
      id: id,
      status: 0
    });
  }
...
const onDelCategory = async (id: string) => {
  ...
  ElMessageBox.confirm("你确定删除吗？", "操作弹框", {
    ...
    .then(async () => {
      // 用户点击“确认”按钮时的回调
      let res = await articleTagCategoryApi.getDelApi(id);
      console.log(res);
      if (res.success) {
        ElMessage({
          type: "success",
...
const getTree = async () => {
  ...
  const res = await articleTagCategoryApi.getTreeApi(pageParams);
...
const getPage = async () => {
  ...
  const res = await articleTagInfoApi.getPageApi(pageParams);
...
const handleDelete = async (id: string) => {
  ...
  ElMessageBox.confirm("你确定删除吗？", "操作弹框", {
    ...
    .then(async () => {
      // 用户点击“确认”按钮时的回调
      let res = await articleTagInfoApi.getDelApi(id);
      console.log(res.success);
      if (res.success) {
        refreshData();
        ElMessage({
