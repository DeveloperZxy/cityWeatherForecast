    ...
    <MePageAsidePanel>
      ...
      <template #main-content>
        <el-table :data="pageList" style="width: 100%; min-height: 400px">
          ...
          <el-table-column prop="content" label="内容" width="130" />
...
import * as articleInfoApi from "@/api/cms/article/articleInfoApi";
...
import SaveInfoForm from "@/views/cms/article/SaveInfoForm.vue";
import SaveCategory from "@/views/cms/article/SaveCategory.vue";
import AdvancedQuery from "@/views/cms/article/AdvancedQuery.vue";
...
const handleStatusChange = async (row: any) => {
  ...
  // 发起编辑状态
  if (status == 0) {
    let res = await articleInfoApi.getSetStatusApi({
      id: id,
      status: 0
    });
  }
...
const onDelCategory = async (id: string) => {
  ...
  ElMessageBox.confirm("你确定删除吗？", "操作弹框", {
    ...
    .then(async () => {
      // 用户点击“确认”按钮时的回调
      let res = await articleCategoryApi.getDelApi(id);
      console.log(res);
      if (res.success) {
        ElMessage({
          type: "success",
...
const getTree = async () => {
  ...
  const res = await articleCategoryApi.getTreeApi(pageParams);
...
const getPage = async () => {
  ...
  const res = await articleInfoApi.getPageApi(pageParams);
...
const handleDelete = async (id: string) => {
  ...
  ElMessageBox.confirm("你确定删除吗？", "操作弹框", {
    ...
    .then(async () => {
      // 用户点击“确认”按钮时的回调
      let res = await articleInfoApi.getDelApi(id);
      console.log(res.success);
      if (res.success) {
        refreshData();
        ElMessage({
