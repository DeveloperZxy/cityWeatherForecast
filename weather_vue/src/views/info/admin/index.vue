<template>
  <div>
    <MePageAsidePanel>
      ...
      <template #main-content>
        <el-table :data="pageList" style="width: 100%; min-height: 400px">
          ...
          <el-table-column prop="categoryId" label="组织" width="130" />
          ...
          <el-table-column label="操作" width="220" fixed="right">
            <template #default="scope">
              ...
              <el-button
                link
                type="primary"
                @click="openPasswordForm(scope.row.id)"
                >修改密码</el-button
              >
            </template>
          </el-table-column>
    ...
    />
    <SetPassword ref="setPasswordFromRef" @success="" />
  </div>
</template>
...
import * as adminCategoryApi from "@/api/info/admin/adminCategoryApi";

import { Edit, Delete, Search, Plus } from "@element-plus/icons-vue";
import SaveInfoForm from "@/views/info/admin/SaveInfoForm.vue";
import SaveCategory from "@/views/info/admin/SaveCategory.vue";
import AdvancedQuery from "@/views/info/admin/AdvancedQuery.vue";
import { ElMessage, ElMessageBox, ElTree } from "element-plus";
import MePageAsidePanel from "@/components/PageLayout/MePageAsidePanel.vue";
import SetPassword from "@/views/info/admin/SetPassword.vue";
...
const handleStatusChange = async (row: any) => {
  ...
  // 发起编辑状态
  if (status == 0) {
    let res = await adminInfoApi.getSetStatusApi({
      id: id,
      status: 0
    });
  }
...
const onDelCategory = async (id: string) => {
  ...
  ElMessageBox.confirm("你确定删除吗？", "操作弹框", {
    ...
    .then(async () => {
      // 用户点击“确认”按钮时的回调
      let res = await adminCategoryApi.getDelApi(id);
      console.log(res);
      if (res.success) {
        ElMessage({
          type: "success",
...
const getTree = async () => {
  ...
  const res = await adminCategoryApi.getTreeApi(pageParams);
...
const getPage = async () => {
  ...
  const res = await adminInfoApi.getPageApi(pageParams);
...
//修改密码的方法
const setPasswordFromRef = ref();
const openPasswordForm = (id: string) => {
  setPasswordFromRef.value.open(id);
};
...
const handleDelete = async (id: string) => {
  ...
  ElMessageBox.confirm("你确定删除吗？", "操作弹框", {
    ...
    .then(async () => {
      // 用户点击“确认”按钮时的回调
      let res = await adminInfoApi.getDelApi(id);
      console.log(res.success);
      if (res.success) {
        refreshData();
        ElMessage({
