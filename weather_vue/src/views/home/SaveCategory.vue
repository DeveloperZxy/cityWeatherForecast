...
import {} from "element-plus/es/locales.mjs";
import { getAddApi, getEditApi, getShowApi } from "@/api/demo/demoCategoryApi";
...
const open = async (type: string, id?: any, categoryId?: any) => {
  //获取树形数据(后端处理的树)
  // const res = await demoCategoryApi.getTreeApi({});
  // categoryList.value = res.data;

  //获取树形数据(前端处理树)    0为不包含Root ，1为包含Root（前后端约定的）
  let response = await demoCategoryApi.getListApi({ includeRoot: "0" });
