    ...
    <MePageAsidePanel>
      ...
      <template #main-content>
        <el-table :data="pageList" style="width: 100%; min-height: 400px">
          ...
          <el-table-column prop="isVip" label="付费会员" />
          <el-table-column prop="userGrade" label="用户等级" />
          <el-table-column prop="isGroup" label="分组" />
...
import * as userInfoApi from "@/api/crm/user/userInfoApi";
import * as userCategoryApi from "@/api/crm/user/userCategoryApi";

import { Edit, Delete, Search, Plus } from "@element-plus/icons-vue";
import SaveInfoForm from "@/views/crm/user/SaveInfoForm.vue";
import SaveCategory from "@/views/crm/user/SaveCategory.vue";
import AdvancedQuery from "@/views/crm/user/AdvancedQuery.vue";
...
const handleStatusChange = async (row: any) => {
  ...
  // 发起编辑状态
  if (status == 0) {
    let res = await userInfoApi.getSetStatusApi({
      id: id,
      status: 0
    });
  }
...
const onDelCategory = async (id: string) => {
  ...
  ElMessageBox.confirm("你确定删除吗？", "操作弹框", {
    ...
    .then(async () => {
      // 用户点击“确认”按钮时的回调
      let res = await userCategoryApi.getDelApi(id);
      console.log(res);
      if (res.success) {
        ElMessage({
          type: "success",
...
const getTree = async () => {
  ...
  const res = await userCategoryApi.getTreeApi(pageParams);
...
const getPage = async () => {
  ...
  const res = await userInfoApi.getPageApi(pageParams);
...
const handleDelete = async (id: string) => {
  ...
  ElMessageBox.confirm("你确定删除吗？", "操作弹框", {
    ...
    .then(async () => {
      // 用户点击“确认”按钮时的回调
      let res = await userInfoApi.getDelApi(id);
      console.log(res.success);
      if (res.success) {
        refreshData();
        ElMessage({
