...
<!--      </el-form-item>-->
    ...
    <ChangePassword ref="changePasswordRorm"/>
  </el-dialog>
</template>
...
//修改密码
const changePasswordRorm = ref();
const changePassword = async () => {
  console.log("修改密码的id", formData.id);
  changePasswordRorm.value.open(formData.id);
};
...
const open = async () => {
  ...
  title.value = "个人中心";
  formData.id = storageLocal().getItem("userId");
  console.log("formData.id", formData.id);
  onEditForm(formData.id);
...
const handleFormSubmit = async () => {
  formData.id = storageLocal().getItem("userId");
  let res = await getEditApi(formData);
  if (res.success) {
    ElMessage.success({
      message: "修改成功",
      type: "success"
