...
const open = async (type: string, id?: any, categoryId?: any) => {
  //获取树形数据(后端处理的树)
  // const res = await dictCategoryApi.getTreeApi({});
  // categoryList.value = res.data;

  //获取树形数据(前端处理树)    0为不包含Root ，1为包含Root（前后端约定的）
  let response = await dictCategoryApi.getListApi({ includeRoot: "0" });
...
const onEditForm = async (id: string) => {
  clearFrom();
  try {
    const res = await dictCategoryApi.getShowApi(id);
    console.log("res", res);
    //将res的值赋给formData
...
const handleFormSubmit = async () => {
  if (title.value === "新增") {
    await dictCategoryApi.getAddApi(formData);
  } else {
    await dictCategoryApi.getEditApi(formData);
  }
  emit("success");
  //表单校验
  dialogShow.value = false;
