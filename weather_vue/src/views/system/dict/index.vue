...
import * as dictCategoryApi from "@/api/system/dict/dictCategoryApi";

import { Edit, Delete, Search, Plus } from "@element-plus/icons-vue";
import SaveInfoForm from "@/views/system/dict/SaveInfoForm.vue";
import SaveCategory from "@/views/system/dict/SaveCategory.vue";
import AdvancedQuery from "@/views/system/dict/AdvancedQuery.vue";
...
const handleStatusChange = async (row: any) => {
  ...
  // 发起编辑状态
  if (status == 0) {
    let res = await dictInfoApi.getSetStatusApi({
      id: id,
      status: 0
    });
  }
...
const onDelCategory = async (id: string) => {
  ...
  ElMessageBox.confirm("你确定删除吗？", "操作弹框", {
    ...
    .then(async () => {
      // 用户点击“确认”按钮时的回调
      let res = await dictCategoryApi.getDelApi(id);
      console.log(res);
      if (res.success) {
        ElMessage({
          type: "success",
...
const getTree = async () => {
  ...
  const res = await dictCategoryApi.getTreeApi(pageParams);
...
const getPage = async () => {
  ...
  const res = await dictInfoApi.getPageApi(pageParams);
...
const handleDelete = async (id: string) => {
  ...
  ElMessageBox.confirm("你确定删除吗？", "操作弹框", {
    ...
    .then(async () => {
      // 用户点击“确认”按钮时的回调
      let res = await dictInfoApi.getDelApi(id);
      console.log(res.success);
      if (res.success) {
        refreshData();
        ElMessage({
