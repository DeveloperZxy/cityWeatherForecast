...
import * as configInfoApi from "@/api/system/config/configInfoApi";
...
import SaveInfoForm from "@/views/system/config/SaveInfoForm.vue";
import SaveCategory from "@/views/system/config/SaveCategory.vue";
import AdvancedQuery from "@/views/system/config/AdvancedQuery.vue";
...
const handleStatusChange = async (row: any) => {
  ...
  // 发起编辑状态
  if (status == 0) {
    let res = await configInfoApi.getSetStatusApi({
      id: id,
      status: 0
    });
  }
...
const onDelCategory = async (id: string) => {
  ...
  ElMessageBox.confirm("你确定删除吗？", "操作弹框", {
    ...
    .then(async () => {
      // 用户点击“确认”按钮时的回调
      let res = await configCategoryApi.getDelApi(id);
      console.log(res);
      if (res.success) {
        ElMessage({
          type: "success",
...
const getTree = async () => {
  ...
  const res = await configCategoryApi.getTreeApi(pageParams);
...
const getPage = async () => {
  ...
  const res = await configInfoApi.getPageApi(pageParams);
...
const handleDelete = async (id: string) => {
  ...
  ElMessageBox.confirm("你确定删除吗？", "操作弹框", {
    ...
    .then(async () => {
      // 用户点击“确认”按钮时的回调
      let res = await configInfoApi.getDelApi(id);
      console.log(res.success);
      if (res.success) {
        refreshData();
        ElMessage({
