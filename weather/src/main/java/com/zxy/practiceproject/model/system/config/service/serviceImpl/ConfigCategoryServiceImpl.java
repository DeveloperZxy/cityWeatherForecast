...
@Service
public class ConfigCategoryServiceImpl extends ServiceImpl<ConfigCategoryMapper, ConfigCategoryEntity> implements ConfigCategoryService {
    @Autowired
     ...
     */
    @Override
    public void add(ConfigCategoryDTO dto) {
        //设置一些基本属性
    ...
    public void del(String id) {
        ...
        boolean exit = configMapper.exists(new LambdaQueryWrapper<ConfigEntity>().eq(ConfigEntity::getCategoryId, id));
        if (exit) {
            //抛出异常
     ...
     */
    @Override
    public void edit(ConfigCategoryDTO configCategoryDTO) {
        ConfigCategoryEntity config = new ConfigCategoryEntity();
        //对象属性拷贝
     ...
     */
    @Override
    public List<ConfigCategoryEntity> listQuery(QueryConfigCategoryDTO queryConfigCategoryDTO) {
        //创建查询种类条件
        LambdaQueryWrapper<ConfigCategoryEntity> queryWrapper = new LambdaQueryWrapper<>();
        //设置查询条件
        if(ObjectUtil.equal(queryConfigCategoryDTO.getIncludeRoot(),"0")){
            queryWrapper.ne(ConfigCategoryEntity::getParentId,"-1");
        }
        //查询
        List<ConfigCategoryEntity> list = configCategoryMapper.selectList(queryWrapper);
        //为空时返回空数组
     ...
     */
    @Override
    public PageResult<PageConfigCategoryDTO> pageQuery(PageQuery<QueryConfigCategoryDTO> dto) {
        //分页查询
        IPage<PageConfigCategoryDTO> page = configCategoryMapper.pageQuery(dto.getPageable(), dto.getFilters());
//        //将每个种类插入ConfigEntity
...
//        }
     ...
     */
    @Override
    public ConfigCategoryDTO show(String id) {
        ConfigCategoryDTO configCategoryDTO = new ConfigCategoryDTO();
        //创建查询条件
        LambdaQueryWrapper<ConfigCategoryEntity> queryWrapper = new LambdaQueryWrapper<>();
        //设置查询条件
        queryWrapper.eq(ConfigCategoryEntity::getId, id);
        //查询
        ConfigCategoryEntity configCategoryEntity = this.getOne(queryWrapper);
        //对象属性拷贝
     ...
     */
    @Override
    public List<Tree<String>> treeQuery(TreeQueryDTO<QueryConfigCategoryDTO> dto) {
        //先查询出符合条件的 所有种类 及其 子类 和 父类
        List<ConfigCategoryEntity> list = configCategoryMapper.listQuery(dto.getFilters());
        List<String> ids = list.stream().map(ConfigCategoryEntity::getId).collect(Collectors.toList());
        ...
        for (ConfigCategoryEntity item : list) {
    ...
    public List<Tree<String>> getTree() {
        //先查询出所有种类
        List<ConfigCategoryEntity> list = this.list();
        //如果种类列表为空，直接返回
        if (CollUtil.isEmpty(list)) {
            return CollUtil.newArrayList();
        }
