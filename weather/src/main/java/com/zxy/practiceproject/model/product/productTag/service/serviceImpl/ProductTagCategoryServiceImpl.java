...
@Service
public class ProductTagCategoryServiceImpl extends ServiceImpl<ProductTagCategoryMapper, ProductTagCategoryEntity> implements ProductTagCategoryService {
    @Autowired
     ...
     */
    @Override
    public void add(ProductTagCategoryDTO dto) {
        //设置一些基本属性
    ...
    public void del(String id) {
        ...
        boolean exit = productTagMapper.exists(new LambdaQueryWrapper<ProductTagEntity>().eq(ProductTagEntity::getCategoryId, id));
        if (exit) {
            //抛出异常
     ...
     */
    @Override
    public void edit(ProductTagCategoryDTO productTagCategoryDTO) {
        ProductTagCategoryEntity productTag = new ProductTagCategoryEntity();
        //对象属性拷贝
        BeanUtils.copyProperties(productTagCategoryDTO, productTag);
        //设置一些基本属性
        productTagCategoryDTO.setParentIds(this.listParentIdsAndSelf(productTagCategoryDTO.getParentId()));
     ...
     */
    @Override
    public List<ProductTagCategoryEntity> listQuery(QueryProductTagCategoryDTO queryConfigCategoryDTO) {
        //创建查询种类条件
        LambdaQueryWrapper<ProductTagCategoryEntity> queryWrapper = new LambdaQueryWrapper<>();
        //设置查询条件
        if(ObjectUtil.equal(queryConfigCategoryDTO.getIncludeRoot(),"0")){
            queryWrapper.ne(ProductTagCategoryEntity::getParentId,"-1");
        }
        //查询
        List<ProductTagCategoryEntity> list = productTagCategoryMapper.selectList(queryWrapper);
        //为空时返回空数组
     ...
     */
    @Override
    public PageResult<PageProductTagCategoryDTO> pageQuery(PageQuery<QueryProductTagCategoryDTO> dto) {
        //分页查询
        IPage<PageProductTagCategoryDTO> page = productTagCategoryMapper.pageQuery(dto.getPageable(), dto.getFilters());
//        //将每个种类插入ConfigEntity
...
//            List<ConfigEntity> productTagEntities = productTagMapper.selectList(queryWrapper1);
//            pageConfigCategoryDTO.setConfigList(productTagEntities);
//        }
        //返回
     ...
     */
    @Override
    public ProductTagCategoryDTO show(String id) {
        ProductTagCategoryDTO productTagCategoryDTO = new ProductTagCategoryDTO();
        //创建查询条件
        LambdaQueryWrapper<ProductTagCategoryEntity> queryWrapper = new LambdaQueryWrapper<>();
        //设置查询条件
        queryWrapper.eq(ProductTagCategoryEntity::getId, id);
        //查询
        ProductTagCategoryEntity productTagCategoryEntity = this.getOne(queryWrapper);
        //对象属性拷贝
        BeanUtils.copyProperties(productTagCategoryEntity, productTagCategoryDTO);
        //返回
        return productTagCategoryDTO;
    }
     ...
     */
    @Override
    public List<Tree<String>> treeQuery(TreeQueryDTO<QueryProductTagCategoryDTO> dto) {
        //先查询出符合条件的 所有种类 及其 子类 和 父类
        List<ProductTagCategoryEntity> list = productTagCategoryMapper.listQuery(dto.getFilters());
        List<String> ids = list.stream().map(ProductTagCategoryEntity::getId).collect(Collectors.toList());
        ...
        for (ProductTagCategoryEntity item : list) {
    ...
    public List<Tree<String>> getTree() {
        //先查询出所有种类
        List<ProductTagCategoryEntity> list = this.list();
        //如果种类列表为空，直接返回
        if (CollUtil.isEmpty(list)) {
            return CollUtil.newArrayList();
        }
