package com.zxy.practiceproject.model.product.productComment.service.serviceImpl;
...
import com.zxy.practiceproject.model.product.productComment.mapper.ProductCommentMapper;
...
@Service
public class ProductCommentServiceImpl extends ServiceImpl<ProductCommentMapper, ProductCommentEntity> implements ProductCommentService {
    @Autowired
     ...
     */
    @Override
    public void add(ProductCommentDTO productTagDTO) {
        //设置一些基本属性
     ...
     */
    @Override
    public void edit(ProductCommentDTO productTagDTO) {
        ProductCommentEntity productTag = new ProductCommentEntity();
        //对象属性拷贝
     ...
     */
    @Override
    public List<ProductCommentEntity> listQuery(QueryProductCommentDTO queryConfigDTO) {
        List<ProductCommentEntity> list = productCommentMapper.listQuery(queryConfigDTO);
        //返回查询结果
        return list;
    }
    ...
    @Override
    public PageResult<PageProductCommentDTO> pageQuery(PageQuery<QueryProductCommentDTO> dto) {
...
//        }
        ...
        //查询分页数据
        IPage<PageProductCommentDTO> page = productCommentMapper.pageQuery(dto.getPageable(), dto.getFilters());
        //返回分页数据
     ...
     */
    @Override
    public ProductCommentDTO show(String id) {
        ProductCommentDTO productTagDTO = new ProductCommentDTO();
        //创建查询条件
        LambdaQueryWrapper<ProductCommentEntity> queryWrapper = new LambdaQueryWrapper<>();
        //设置查询条件
        queryWrapper.eq(ProductCommentEntity::getId, id);
        //查询
        ProductCommentEntity productTagEntity = this.getOne(queryWrapper);
        //对象属性拷贝
    ...
    public void setStatus(String id, Integer status) {
        //创建更新条件
        LambdaUpdateWrapper<ProductCommentEntity> updateWrapper = new LambdaUpdateWrapper<>();
        //设置更新条件
        updateWrapper.eq(ProductCommentEntity::getId, id);
        updateWrapper.set(ProductCommentEntity::getStatus, status);
        //更新状态
        this.update(updateWrapper);
    }

    @Override
    public List<Tree<String>> treeQuery(TreeQueryDTO<QueryProductCommentDTO> dto) {
        //先查询出符合条件的 所有种类 及其 子类 和 父类
        List<ProductCommentEntity> list = this.listQuery(dto.getFilters());
        List<String> ids = list.stream().map(ProductCommentEntity::getId).collect(Collectors.toList());
        ...
        for (ProductCommentEntity item : list) {
    ...
    public List<Tree<String>> getTree() {
        //先查询出所有种类
        List<ProductCommentEntity> list = this.list();
        //如果种类列表为空，直接返回
        if (CollUtil.isEmpty(list)) {
            return CollUtil.newArrayList();
        }
