package com.zxy.practiceproject.model.product.productCategory.service.serviceImpl;
...
import com.zxy.practiceproject.model.product.productCategory.mapper.ProductCategoryMapper;
...
public class ProductCategoryServiceImpl extends ServiceImpl<ProductCategoryMapper, ProductCategoryEntity> implements ProductCategoryService {
    @Autowired
    private ProductCategoryMapper productTagMapper;
     ...
     */
    @Override
    public void add(ProductCategoryDTO productTagDTO) {
        //设置一些基本属性
     ...
     */
    @Override
    public void edit(ProductCategoryDTO productTagDTO) {
        ProductCategoryEntity productTag = new ProductCategoryEntity();
        //对象属性拷贝
     ...
     */
    @Override
    public List<ProductCategoryEntity> listQuery(QueryProductCategoryDTO queryConfigDTO) {
        List<ProductCategoryEntity> list = productTagMapper.listQuery(queryConfigDTO);
        //返回查询结果
        return list;
    }
    ...
    @Override
    public PageResult<PageProductCategoryDTO> pageQuery(PageQuery<QueryProductCategoryDTO> dto) {
...
//        }
        ...
        //查询分页数据
        IPage<PageProductCategoryDTO> page = productTagMapper.pageQuery(dto.getPageable(), dto.getFilters());
        //返回分页数据
     ...
     */
    @Override
    public ProductCategoryDTO show(String id) {
        ProductCategoryDTO productTagDTO = new ProductCategoryDTO();
        //创建查询条件
        LambdaQueryWrapper<ProductCategoryEntity> queryWrapper = new LambdaQueryWrapper<>();
        //设置查询条件
        queryWrapper.eq(ProductCategoryEntity::getId, id);
        //查询
        ProductCategoryEntity productTagEntity = this.getOne(queryWrapper);
        //对象属性拷贝
    ...
    public void setStatus(String id, Integer status) {
        //创建更新条件
        LambdaUpdateWrapper<ProductCategoryEntity> updateWrapper = new LambdaUpdateWrapper<>();
        //设置更新条件
        updateWrapper.eq(ProductCategoryEntity::getId, id);
        updateWrapper.set(ProductCategoryEntity::getStatus, status);
        //更新状态
        this.update(updateWrapper);
    }

    @Override
    public List<Tree<String>> treeQuery(TreeQueryDTO<QueryProductCategoryDTO> dto) {
        //先查询出符合条件的 所有种类 及其 子类 和 父类
        List<ProductCategoryEntity> list = this.listQuery(dto.getFilters());
        List<String> ids = list.stream().map(ProductCategoryEntity::getId).collect(Collectors.toList());
        ...
        for (ProductCategoryEntity item : list) {
