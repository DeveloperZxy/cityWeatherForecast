package com.zxy.practiceproject.model.system.permission.service.serviceImpl;
...
import com.zxy.practiceproject.model.system.permission.mapper.PermissionInfoMapper;
...
import com.zxy.practiceproject.model.system.permission.pojo.entity.PermissionMeta;
...
@Service
public class PermissionInfoServiceImpl extends ServiceImpl<PermissionInfoMapper, PermissionInfoEntity> implements PermissionInfoService {
    @Autowired
     ...
     */
    @Override
    public void add(PermissionInfoDTO permissionInfoDTO) {
        //设置一些基本属性
        PermissionMeta permissionMeta = new PermissionMeta();
        permissionMeta.setTitle(permissionInfoDTO.getName());
        permissionInfoDTO.setMeta(permissionMeta);
       //
     ...
     */
    @Override
    public void edit(PermissionInfoDTO permissionInfoDTO) {
        PermissionInfoEntity permissionInfo = new PermissionInfoEntity();
        //对象属性拷贝
     ...
     */
    @Override
    public List<PermissionInfoEntity> listQuery(QueryPermissionInfoDTO queryConfigDTO) {
        List<PermissionInfoEntity> list = permissionInfoMapper.listQuery(queryConfigDTO);
        //返回查询结果
        return list;
    }
    ...
    @Override
    public PageResult<PagePermissionInfoDTO> pageQuery(PageQuery<QueryPermissionInfoDTO> dto) {
...
//        }
        ...
        //查询分页数据
        IPage<PagePermissionInfoDTO> page = permissionInfoMapper.pageQuery(dto.getPageable(), dto.getFilters());
        //返回分页数据
     ...
     */
    @Override
    public PermissionInfoDTO show(String id) {
        ...
        //设置查询条件
        queryWrapper.eq(PermissionInfoEntity::getId, id);
        //查询
        PermissionInfoEntity permissionInfoEntity = this.getOne(queryWrapper);
        //对象属性拷贝
    ...
    public void setStatus(String id, Integer status) {
        //创建更新条件
        LambdaUpdateWrapper<PermissionInfoEntity> updateWrapper = new LambdaUpdateWrapper<>();
        //设置更新条件
        updateWrapper.eq(PermissionInfoEntity::getId, id);
        updateWrapper.set(PermissionInfoEntity::getStatus, status);
        //更新状态
        this.update(updateWrapper);
    }

    @Override
    public List<Tree<String>> treeQuery(TreeQueryDTO<QueryPermissionInfoDTO> dto) {
        //先查询出符合条件的 所有种类 及其 子类 和 父类
        List<PermissionInfoEntity> list = this.listQuery(dto.getFilters());
        List<String> ids = list.stream().map(PermissionInfoEntity::getId).collect(Collectors.toList());
        ...
        for (PermissionInfoEntity item : list) {
    ...
    public List<String> listParentIdsAndSelf(String id) {
        List<Tree<String>> trees = this.getTree("0");

   return   listParentIdsAndSelf(id,trees);
    }
     ...
     * @return
     */
    public List<String> listParentIdsAndSelf(String id, List<Tree<String>> trees) {
    ...
    public List<Tree<String>> getTree(String includeRoot) {
        //先查询出所有种类
        List<PermissionInfoEntity> list = this.list();
        //如果种类列表为空，直接返回
        if (CollUtil.isEmpty(list)) {
            return CollUtil.newArrayList();
        }
