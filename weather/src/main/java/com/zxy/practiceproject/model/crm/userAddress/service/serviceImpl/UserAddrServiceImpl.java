package com.zxy.practiceproject.model.crm.userAddress.service.serviceImpl;
...
import com.zxy.practiceproject.model.crm.userAddress.mapper.UserAddrMapper;
...
@Service
public class UserAddrServiceImpl extends ServiceImpl<UserAddrMapper, UserAddrEntity> implements UserAddrService {
    @Autowired
    private UserAddrMapper productTagMapper;
     ...
     */
    @Override
    public void add(UserAddrDTO productTagDTO) {
        productTagDTO.setUserId(JwtUtil.getLoginUserId());
        //保存
     ...
     */
    @Override
    public void edit(UserAddrDTO productTagDTO) {
        UserAddrEntity productTag = new UserAddrEntity();
        //对象属性拷贝
     ...
     */
    @Override
    public List<UserAddrEntity> listQuery(QueryUserAddrDTO queryConfigDTO) {
        queryConfigDTO.setUserId(JwtUtil.getLoginUserId());
        List<UserAddrEntity> list = productTagMapper.listQuery(queryConfigDTO);
        //返回查询结果
        return list;
    }
    ...
    @Override
    public PageResult<PageUserAddrDTO> pageQuery(PageQuery<QueryUserAddrDTO> dto) {
...
//        }
        ...
        //查询分页数据
        IPage<PageUserAddrDTO> page = productTagMapper.pageQuery(dto.getPageable(), dto.getFilters());
        //返回分页数据
     ...
     */
    @Override
    public UserAddrDTO show(String id) {
        UserAddrDTO productTagDTO = new UserAddrDTO();
        //创建查询条件
        LambdaQueryWrapper<UserAddrEntity> queryWrapper = new LambdaQueryWrapper<>();
        //设置查询条件
        queryWrapper.eq(UserAddrEntity::getId, id);
        //查询
        UserAddrEntity productTagEntity = this.getOne(queryWrapper);
        //对象属性拷贝
    ...
    public void setStatus(String id, Integer status) {
        //创建更新条件
        LambdaUpdateWrapper<UserAddrEntity> updateWrapper = new LambdaUpdateWrapper<>();
        //设置更新条件
        updateWrapper.eq(UserAddrEntity::getId, id);
        updateWrapper.set(UserAddrEntity::getStatus, status);
        //更新状态
        this.update(updateWrapper);
    }

    @Override
    public List<Tree<String>> treeQuery(TreeQueryDTO<QueryUserAddrDTO> dto) {
        //先查询出符合条件的 所有种类 及其 子类 和 父类
        List<UserAddrEntity> list = this.listQuery(dto.getFilters());
        List<String> ids = list.stream().map(UserAddrEntity::getId).collect(Collectors.toList());
     ...
     */
    @Override
    public OrderDetailsVO getOrderDetails(UserAddrDTO userAddrDTO) {
    ...
    public List<Tree<String>> getTree(String includeRoot) {
        //先查询出所有种类
        List<UserAddrEntity> list = this.list();
        //如果种类列表为空，直接返回
        if (CollUtil.isEmpty(list)) {
            return CollUtil.newArrayList();
        }
