...
public class ArticleServiceImpl extends ServiceImpl<ArticleMapper, ArticleEntity> implements ArticleService {
     ...
     */
    @Override
    public List<ArticleEntity> listQuery(QueryArticleDTO queryConfigDTO) {
        //创建查询条件
        LambdaQueryWrapper<ArticleEntity> queryWrapper = new LambdaQueryWrapper<>();
        //设置查询条件并查询
        List<ArticleEntity> list = articleMapper.selectList(queryWrapper);
        //返回查询结果
        return list;
    }
    ...
    @Override
    public PageResult<PageArticleDTO> pageQuery(PageQuery<QueryArticleDTO> dto) {
...
//        }
        ...
        if(StrUtil.isNotBlank(categoryId)){
            List<String> categoryIds = articleCategoryMapper.listChildrenIdsAndSelf(categoryId);
            //将这些id 存在拦截器条件里
            dto.getFilters().setCategoryIds(categoryIds);
        }

        //查询分页数据
        IPage<PageArticleDTO> page = articleMapper.pageQuery(dto.getPageable(), dto.getFilters());
        //返回分页数据
     ...
     */
    @Override
    public ArticleDTO show(String id) {
        ArticleDTO articleDTO = new ArticleDTO();
        //创建查询条件
        LambdaQueryWrapper<ArticleEntity> queryWrapper = new LambdaQueryWrapper<>();
        //设置查询条件
        queryWrapper.eq(ArticleEntity::getId, id);
        //查询
    ...
    public void setStatus(String id, Integer status) {
        ...
        //设置更新条件
        updateWrapper.eq(ArticleEntity::getId, id);
        updateWrapper.set(ArticleEntity::getStatus, status);
        //更新状态
