package com.zxy.practiceproject.model.product.productBrand.service.serviceImpl;
...
import com.zxy.practiceproject.model.product.productBrand.mapper.ProductBrandMapper;
...
import com.zxy.practiceproject.model.product.productBrand.service.ProductBrandService;
...
public class ProductBrandServiceImpl extends ServiceImpl<ProductBrandMapper, ProductBrandEntity> implements ProductBrandService {
    @Autowired
    private ProductBrandMapper productTagMapper;
     ...
     */
    @Override
    public void add(ProductBrandDTO productTagDTO) {
        //设置一些基本属性
     ...
     */
    @Override
    public void edit(ProductBrandDTO productTagDTO) {
        ProductBrandEntity productTag = new ProductBrandEntity();
        //对象属性拷贝
     ...
     */
    @Override
    public List<ProductBrandEntity> listQuery(QueryProductBrandDTO queryConfigDTO) {
        List<ProductBrandEntity> list = productTagMapper.listQuery(queryConfigDTO);
        //返回查询结果
        return list;
    }
    ...
    @Override
    public PageResult<PageProductBrandDTO> pageQuery(PageQuery<QueryProductBrandDTO> dto) {
...
//        }
        ...
        //查询分页数据
        IPage<PageProductBrandDTO> page = productTagMapper.pageQuery(dto.getPageable(), dto.getFilters());
        //返回分页数据
     ...
     */
    @Override
    public ProductBrandDTO show(String id) {
        ProductBrandDTO productTagDTO = new ProductBrandDTO();
        //创建查询条件
        LambdaQueryWrapper<ProductBrandEntity> queryWrapper = new LambdaQueryWrapper<>();
        //设置查询条件
        queryWrapper.eq(ProductBrandEntity::getId, id);
        //查询
        ProductBrandEntity productTagEntity = this.getOne(queryWrapper);
        //对象属性拷贝
    ...
    public void setStatus(String id, Integer status) {
        //创建更新条件
        LambdaUpdateWrapper<ProductBrandEntity> updateWrapper = new LambdaUpdateWrapper<>();
        //设置更新条件
        updateWrapper.eq(ProductBrandEntity::getId, id);
        updateWrapper.set(ProductBrandEntity::getStatus, status);
        //更新状态
        this.update(updateWrapper);
    }

    @Override
    public List<Tree<String>> treeQuery(TreeQueryDTO<QueryProductBrandDTO> dto) {
        //先查询出符合条件的 所有种类 及其 子类 和 父类
        List<ProductBrandEntity> list = this.listQuery(dto.getFilters());
        List<String> ids = list.stream().map(ProductBrandEntity::getId).collect(Collectors.toList());
    ...
    public List<Tree<String>> getTree(String includeRoot) {
        //先查询出所有种类
        List<ProductBrandEntity> list = this.list();
        //如果种类列表为空，直接返回
        if (CollUtil.isEmpty(list)) {
            return CollUtil.newArrayList();
        }
