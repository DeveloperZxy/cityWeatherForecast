...
import com.zxy.practiceproject.model.user.shop.productInfo.mapper.ProductRecordMapper;
...
@Service
public class ProductRecordService extends ServiceImpl<ProductRecordMapper, ProductRecordEntity> implements com.zxy.practiceproject.model.user.shop.productInfo.service.ProductRecordService {
    @Autowired
    private ProductRecordMapper productTagMapper;
     ...
     */
    @Override
    public void add(ProductRecordDTO productTagDTO) {
     ...
     */
    @Override
    public void edit(ProductRecordDTO productTagDTO) {
        ProductRecordEntity productTag = new ProductRecordEntity();
        //对象属性拷贝
     ...
     */
    @Override
    public List<ProductRecordEntity> listQuery(QueryProductRecordDTO queryConfigDTO) {
        List<ProductRecordEntity> list = productTagMapper.listQuery(queryConfigDTO);
        //返回查询结果
        return list;
    }
    ...
    @Override
    public PageResult<PageProductRecordDTO> pageQuery(PageQuery<QueryProductRecordDTO> dto) {
...
//        }
        ...
        //查询分页数据
        IPage<PageProductRecordDTO> page = productTagMapper.pageQuery(dto.getPageable(), dto.getFilters());
        //返回分页数据
     ...
     */
    @Override
    public ProductRecordDTO show(String id) {
        ProductRecordDTO productTagDTO = new ProductRecordDTO();
        //创建查询条件
        LambdaQueryWrapper<ProductRecordEntity> queryWrapper = new LambdaQueryWrapper<>();
        //设置查询条件
        queryWrapper.eq(ProductRecordEntity::getId, id);
        //查询
        ProductRecordEntity productTagEntity = this.getOne(queryWrapper);
        //对象属性拷贝
    ...
    public void setStatus(String id, Integer status) {
        //创建更新条件
        LambdaUpdateWrapper<ProductRecordEntity> updateWrapper = new LambdaUpdateWrapper<>();
        //设置更新条件
        updateWrapper.eq(ProductRecordEntity::getId, id);
        updateWrapper.set(ProductRecordEntity::getStatus, status);
        //更新状态
        this.update(updateWrapper);
    }

    @Override
    public List<Tree<String>> treeQuery(TreeQueryDTO<QueryProductRecordDTO> dto) {
        //先查询出符合条件的 所有种类 及其 子类 和 父类
        List<ProductRecordEntity> list = this.listQuery(dto.getFilters());
        List<String> ids = list.stream().map(ProductRecordEntity::getId).collect(Collectors.toList());
        ...
        for (ProductRecordEntity item : list) {
    ...
    public List<Tree<String>> getTree(String includeRoot) {
        //先查询出所有种类
        List<ProductRecordEntity> list = this.list();
        //如果种类列表为空，直接返回
        if (CollUtil.isEmpty(list)) {
            return CollUtil.newArrayList();
        }
