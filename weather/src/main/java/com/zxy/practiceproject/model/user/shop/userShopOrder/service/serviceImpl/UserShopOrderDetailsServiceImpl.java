...
import com.zxy.practiceproject.model.user.shop.userShopOrder.mapper.UserShopOrderDetailsMapper;
...
@Service
public class UserShopOrderDetailsServiceImpl extends ServiceImpl<UserShopOrderDetailsMapper, UserShopOrderDetailsEntity> implements UserShopOrderDetailsService {
    @Autowired
    private UserShopOrderDetailsMapper productTagMapper;
    @Autowired
     ...
     */
    @Override
    public void add(CartListDTO cartListDTO) {

        //将购物车数据保存到订单中
        for(String cartId : cartListDTO.getCartIds()){
            UserShopOrderDetailsEntity orderDetails = new UserShopOrderDetailsEntity();
            UserShopCartEntity cart = userShopCartService.getById(cartId);

            orderDetails.setProductId(cart.getProductId());
            orderDetails.setNum(cart.getNum());
            orderDetails.setUserId(cart.getUserId());
            orderDetails.setProductDetailsId(cart.getProductDetailsId());
            ...
            ProductRecordEntity product = productRecordService.getById(cart.getProductDetailsId());
            orderDetails.setWeight(product.getWeight());
            orderDetails.setVolume(product.getVolume());


            orderDetails.setOrderId(cartListDTO.getOrderId());
     ...
     */
    @Override
    public void edit(UserShopOrderDetailsDTO productTagDTO) {
        UserShopOrderDetailsEntity productTag = new UserShopOrderDetailsEntity();
        //对象属性拷贝
     ...
     */
    @Override
    public List<UserShopOrderDetailsEntity> listQuery(QueryUserShopOrderDetailsDTO queryConfigDTO) {
        List<UserShopOrderDetailsEntity> list = productTagMapper.listQuery(queryConfigDTO);
        //返回查询结果
        return list;
    }
    ...
    @Override
    public PageResult<PageUserShopOrderDetailsDTO> pageQuery(PageQuery<QueryUserShopOrderDetailsDTO> dto) {
...
//        }
        ...
        //查询分页数据
        IPage<PageUserShopOrderDetailsDTO> page = productTagMapper.pageQuery(dto.getPageable(), dto.getFilters());
        //返回分页数据
     ...
     */
    @Override
    public UserShopOrderDetailsDTO show(String id) {
        UserShopOrderDetailsDTO productTagDTO = new UserShopOrderDetailsDTO();
        //创建查询条件
        LambdaQueryWrapper<UserShopOrderDetailsEntity> queryWrapper = new LambdaQueryWrapper<>();
        //设置查询条件
        queryWrapper.eq(UserShopOrderDetailsEntity::getId, id);
        //查询
        UserShopOrderDetailsEntity productTagEntity = this.getOne(queryWrapper);
        //对象属性拷贝
    ...
    public void setStatus(String id, Integer status) {
        //创建更新条件
        LambdaUpdateWrapper<UserShopOrderDetailsEntity> updateWrapper = new LambdaUpdateWrapper<>();
        //设置更新条件
        updateWrapper.eq(UserShopOrderDetailsEntity::getId, id);
        updateWrapper.set(UserShopOrderDetailsEntity::getStatus, status);
        //更新状态
        this.update(updateWrapper);
    }

    @Override
    public List<Tree<String>> treeQuery(TreeQueryDTO<QueryUserShopOrderDetailsDTO> dto) {
        //先查询出符合条件的 所有种类 及其 子类 和 父类
        List<UserShopOrderDetailsEntity> list = this.listQuery(dto.getFilters());
        List<String> ids = list.stream().map(UserShopOrderDetailsEntity::getId).collect(Collectors.toList());
        ...
        for (UserShopOrderDetailsEntity item : list) {
    ...
    public List<Tree<String>> getTree(String includeRoot) {
        //先查询出所有种类
        List<UserShopOrderDetailsEntity> list = this.list();
        //如果种类列表为空，直接返回
        if (CollUtil.isEmpty(list)) {
            return CollUtil.newArrayList();
        }
